...
```{r ks_tests_all, warning=F, message=F, echo=F, fig.width=10, fig.height=5}
library("stats")
...
```{r cdf_234, warning=F, message=F, echo=F, fig.width=10, fig.height=5}
# To compare DG for A and LG for B we need to rename the districts to easily facet wrap them:
df_all_cdf <- df_all_cdf %>%
  mutate(District.compare = ifelse((District == "EDG" & Player == 'A')|(District == "ELG" & Player == 'B'), "DG Player A to LG Player B",
                                   ifelse((District == "ELG" & Player == 'A')|(District == "EDG" & Player == 'B'), "LG Player A to DG Player B","W for Both Players")),
...
symm.1.1.cdf.all <- ggplot(df_all_cdf %>% 
         filter(Map == 2, District == c("EDG","ELG","EW")), aes(x = as.numeric(Effort))) +
...
label = c(sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[2,2]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[3,2]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[1,2]))
...
symm.1.3.cdf.all <- ggplot(df_all_cdf %>% 
         filter(Map == 3, District == c("EDG","ELG","EW")), aes(x = as.numeric(Effort))) +
  stat_ecdf(aes(colour=Player)) +
  scale_color_manual(values = c("Gray", "Black")) +
  facet_wrap(~District.compare)+
...
label = c(sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[2,3]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[3,3]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[1,3]))
data_text <- data.frame(District.compare = c("DG Player A to LG Player B","LG Player A to DG Player B","W for Both Players"),
                        label,
                        x = c(60, 60, 60),
                        y = c(0.25, 0.25, 0.25),
...
symm.3.1.cdf.all <- ggplot(df_all_cdf %>% 
         filter(Map == 4, District == c("EDG","ELG","EW")), aes(x = as.numeric(Effort))) +
  stat_ecdf(aes(colour=Player)) +
  scale_color_manual(values = c("Gray", "Black")) +
  facet_wrap(~District.compare)+
...
label = c(sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[2,4]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[3,4]), sprintf("italic(%s) == %.4f", "p-value", sym.k.s.tests[1,4]))
data_text <- data.frame(District.compare = c("DG Player A to LG Player B","LG Player A to DG Player B","W for Both Players"),
                        label,
                        x = c(60, 60, 60),
                        y = c(0.25, 0.25, 0.25),
...
gerry_adv.cdf.all <- ggplot(df_all_cdf %>% 
         filter(Advantage == "Adv", District == c("EDG","ELG","EW")), aes(x = as.numeric(Effort))) +
  stat_ecdf(aes(colour=Player)) +
  scale_color_manual(values = c("Gray", "Black")) +
...
label = c(sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[1,2]), sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[2,2]), sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[3,2]))
...
gerry_disadv.cdf.all <- ggplot(df_all_cdf %>% 
         filter(Advantage == "Dis.adv", District == c("EDG","ELG","EW")), aes(x = as.numeric(Effort))) +
  stat_ecdf(aes(colour=Player)) +
  scale_color_manual(values = c("Gray", "Black")) +
  ...
  guides(size = F)
label = c(sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[1,3]), sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[2,3]), sprintf("italic(%s) == %.4f", "p-value", adv.disadv.ks.tests[3,3]))
...
player.role.regression.data <- df_all_cdf %>% 
  filter(District == c("EDG", "ELG", "EW")) %>%
  mutate(PlayerB_Dummy = ifelse(Player == "A", 0, 1))
...
library(estimatr)
temp2 <- lm_robust(Effort ~ PlayerB_Dummy + subject.id, clusters = Session, data = player.role.regression.data)
#summary(temp2)
```
