...
ggplot(only.4, aes(x = as.numeric(Effort), size = "1")) + 
  ...
  facet_wrap(~Player) +
  ggtitle("Disaggregated Map 4") + guides(size = F)
```
...
ggplot(dissag.df.overlay %>% filter(Advantage == "Adv"), aes(x=as.numeric(Effort), size = "1")) + 
  stat_ecdf(aes(colour=Player)) + 
  ggtitle("Disaggregated Advantage Comparrison: White District") + 
  guides(size = F)

ggplot(dissag.df.overlay %>% filter(Advantage == "Dis.adv"), aes(x=as.numeric(Effort), size = "1")) + 
  stat_ecdf(aes(colour=Player)) + 
  ggtitle("Disaggregated Disadvantage Comparrison: White District") + 
  guides(size = F)
```
...
<!--   stat_ecdf(aes(colour=Advantage)) + -->
<!--   ggtitle("Advnataged vs Disadvantaged") + -->
<!--   guides(size = F) -->
<!-- cdf_adv_dis.adv_overlay + scale_fill_brewer() -->
<!-- ``` -->
