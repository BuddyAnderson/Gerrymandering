...
[]- Regression from Deck's notes

[]- As a first pass, we should run a K-S tests to see if the various pairs of distributions you overlaid are the same. 
...
session.level.ts.avgs <- time_series_avg %>% filter(Period < 28, District == c("EW","EDG","ELG")) %>% 
  group_by(Session, Period, Map, District, Advantage) %>% summarise(avg.Effort = mean(as.numeric(Effort)))
...
[]- Rank sum test looking at whether or not their political views influence whether they gerrymander or not...?
[]- relationships 
- political beliefs and saying gerrymandering 
- how either of those answers depend on whether they actually gerrymander
- T test of whether observations are same for number of people selecting whether they supprot gerrymandering or not

- Look at for gerrymanderers, is their political response different thatn the political response of the non-gerrymanderers
- for the same split, did they say they like gerrymandering or not (are the porportions the same)

- comparing the two survey responses; the relationship between gerrymandering and political views
