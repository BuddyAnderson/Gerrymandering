...
ts.each.map.each.dist %>% filter(District == c("EW","EDG", "ELG")) %>% group_by(Period, Map, District) %>% summarise(avg.Effort = mean(Effort)) %>%
  ggplot(aes(x=Period, y=avg.Effort, color = District)) +
  geom_line() + xlim(1,10) + ylim(0,60) +
  scale_x_continuous(breaks=c(1:10)) + facet_wrap(~Map)
```
...
pct.zero <- long_df %>% filter(Period <= 24 & Period >= 20) %>%
  group_by(District) %>% summarize(pct.bid.zero = round(sum(Effort == 0)/n(), 2))
pct.zero

# pct.stage_1.zero <- stage_1
# pct.stage_1.zero[pct.stage_1.zero == 0] <- NA
# summarize(pct.stage_1.zero,
#           pct0.EDG.5 = sum(is.na(EDG_5))/n(),
#           pct0.ELG.5 = sum(is.na(ELG_5))/n(),
#           pct0.EW.5 = sum(is.na(EW_5))/n(),
#           pct0.EDG.4 = sum(is.na(EDG_4))/n(),
#           pct0.ELG.4 = sum(is.na(ELG_4))/n(),
#           pct0.EW.4 = sum(is.na(EW_4))/n(),
#           pct0.EDG.3 = sum(is.na(EDG_3))/n(),
#           pct0.ELG.3 = sum(is.na(ELG_3))/n(),
#           pct0.EW.3 = sum(is.na(EW_3))/n(),
#           pct0.EDG.2 = sum(is.na(EDG_2))/n(),
#           pct0.ELG.2 = sum(is.na(ELG_2))/n(),
#           pct0.EW.2 = sum(is.na(EW_2))/n(),
#           pct0.EDG.1 = sum(is.na(EDG_1))/n(),
#           pct0.ELG.1 = sum(is.na(ELG_1))/n(),
#           pct0.EW.1 = sum(is.na(EW_1))/n()
#           )
```
